...
build_from_image() {
    ...
    # Apply LXC/Docker specific changes to the unpacked boot image or ramdisk
    if [[ "$1" == *"lxc"* ]]; then
        echo '[+] Applying LXC/Docker specific changes'
        # Modify the init scripts or add necessary binaries and libraries for LXC/Docker support
        # For example, adding LXC/Docker binaries, modifying init scripts, etc.
        # This can be done by mounting the ramdisk and copying files into it.
        # Example command (modify as needed):
        # cp -r /path/to/lxc-docker-files/* out/ramdisk/
        # Please ensure you have the correct paths and files.
    fi
